...
func (a *Agent) collectProfile(ctx context.Context, ptype profile.ProfileType, buf *bytes.Buffer) error {
	...
	case profile.TypeTrace:
		err := trace.Start(buf)
		if err != nil {
			return fmt.Errorf("failed to start trace profile: %v", err)
		}
		sleep(a.TraceProfileDuration, ctx.Done())
		trace.Stop()
	case profile.TypeBlock,
		profile.TypeMutex,
		profile.TypeGoroutine,
		profile.TypeThreadcreate:
		// TODO: если блок или мьютекс, нужно включить runtime.SetBlockProfileRate()
